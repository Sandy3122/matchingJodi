$(document).ready(function () {
  ...
  $("#searchBtn").click(function () {
    ...
    // Function to get token value from session by type (adminToken or employeeToken)
    function getTokenFromSession(tokenType) {
      if (tokenType === "adminToken") {
        return sessionStorage.getItem("adminToken");
      } else if (tokenType === "employeeToken") {
    ...
    $.ajax({
      ...
      beforeSend: function (xhr) {
        var adminToken = getTokenFromSession("adminToken");
        var employeeToken = getTokenFromSession("employeeToken");
        if (adminToken) {
          xhr.setRequestHeader("Authorization", "Bearer " + adminToken);
        } else if (employeeToken) {
          xhr.setRequestHeader("Authorization", "Bearer " + employeeToken);
        }
      },
